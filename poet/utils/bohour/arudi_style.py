...
shadda_chars = ["\u0651"]
all_chars = list("إةابتثجحخدذرزسشصضطظعغفقكلمنهويىأءئؤ #ـ")
    ...
    while i < len(chars) - 1 and flag:
        ...
        if char in all_chars:
            ...
            if next_char in harakat:
                ...
                # Handle # character - check if it indicates end of shatr
                if i > 0 and chars[i-1] == "ـ":
                    # Previous character is tatweel, don't add 0
                    pass
                else:
                    # Not preceded by tatweel, check if we need to add 0 (only if last output is not 0)
                    if len(out) > 0 and out[-1] != "0":
                        out += "0"  # Add 0 to indicate end of shatr
                # Skip the # character and continue to next character
                i += 1
                continue
            ...
            i += 2
        if j > 2 * len(chars):
            print(f"Warning: Loop exceeded limit for text: {pred[:50]}...")
            flag = False
            break
