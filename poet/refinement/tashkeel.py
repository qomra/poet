...
class TashkeelRefiner(Node):
    """
    Refines poem tashkeel (diacritics) to improve pronunciation.
    ...
    def run(self, input_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Check if tashkeel refinement is needed
        if not self.should_refine(evaluation):
            self.logger.info("ℹ️ Tashkeel refinement not needed")
            return {
                'poem': poem,
                'refined': False,
                'refiner_used': 'tashkeel_refiner'
            }
        
        # Perform tashkeel refinement
        refined_poem = self._refine_tashkeel(poem, constraints, evaluation)
    ...
    def should_refine(self, evaluation: QualityAssessment) -> bool:
        """Check if tashkeel refinement is needed."""
        if not evaluation or not evaluation.tashkeel_validation:
            return False
        
        return not evaluation.tashkeel_validation.overall_valid
    
    def _refine_tashkeel(self, poem: LLMPoem, constraints: Constraints, evaluation: QualityAssessment) -> LLMPoem:
        """Refine the poem's tashkeel."""
        self.logger.info("🔤 Refining poem tashkeel")
        
        # Get tashkeel issues from evaluation
        tashkeel_issues = []
        if evaluation.tashkeel_validation and evaluation.tashkeel_validation.issues:
            tashkeel_issues = evaluation.tashkeel_validation.issues
        
        # Create refinement prompt
        refinement_prompt = self.prompt_manager.format_prompt(
            'tashkeel_refinement',
            poem_text=poem.get_text(),
            tashkeel_issues='\n'.join(tashkeel_issues) if tashkeel_issues else "No specific issues identified",
        ...
        self.logger.info("✅ Tashkeel refinement completed")
        return refined_poem
    ...
    def _generate_reasoning(self, input_data: Dict[str, Any], output_data: Dict[str, Any]) -> str:
        ...
        if refined:
            reasoning = f"I refined the poem's tashkeel{iteration_text}."
            reasoning += " I improved the diacritical marks to enhance pronunciation and clarity."
        else:
            reasoning = f"I checked the poem's tashkeel{iteration_text}."
            reasoning += " The tashkeel was already acceptable, so no refinement was needed."
    ...
    def _summarize_input(self) -> str:
        ...
        if poem:
            return f"Refined tashkeel for poem with {len(poem.verses)} verses"
        return "Refined tashkeel"
    ...
    def _summarize_output(self) -> str:
        ...
        return f"Tashkeel refinement: {'Applied' if refined else 'Not needed'}"
